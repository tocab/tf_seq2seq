...
from tensorflow.contrib.seq2seq import TrainingHelper, dynamic_decode, BahdanauAttention, DynamicAttentionWrapper, \
    BasicDecoder, DynamicAttentionWrapperState
from tensorflow.python.layers import core as layers_core
from helper.calc_bleu_score import bleu_score
import sys
...
class seq2seq_model:
    def __init__(self, word_index_1, word_index_2, weights, encoder_hidden_units=500, decoder_hidden_units=500,
        ...
        # Re-arrange encoder final states since they are time major:
        # [batch_size, max_time, num_units] -> [max_time, batch_size, num_units]
        attention_states = tf.transpose(outputs, [1, 0, 2])

        # Create an attention mechanism
        attention_mechanism = BahdanauAttention(512, attention_states,
                                                memory_sequence_length=self.encoder_inputs_length)

        decoder_cell = DynamicAttentionWrapper(LSTMCell(decoder_hidden_units), attention_mechanism, 512)
        ...
        attn_zero = decoder_cell.zero_state(batch_size=batch_size, dtype=tf.float32)
        attn_zero = DynamicAttentionWrapperState(self.encoder_final_state, attn_zero.attention)
        #init_state = attn_zero.clone(cell_state=self.encoder_final_state)
        decoder = BasicDecoder(decoder_cell, helper, attn_zero, output_layer=projection_layer)
    ...
    def train(self, iterations, src_data, src_len, tgt_input, tgt_output, tgt_len, train_ratio=0.8):
        ...
        for batch in range(iterations):
            ...
            if batch % 100 == 0:
                ...
                # Calc bleu score
                train_bleu = bleu_score(train_batch_tgt_input, train_output)
                ...
                # Calc bleu score
                eval_bleu = bleu_score(eval_batch_tgt_input, eval_output)
